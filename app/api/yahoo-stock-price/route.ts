...
export async function POST(request: Request) {
  try {
    ...
    const oneDayChange = meta.regularMarketChange || (currentPrice - previousClose)
    const oneDayChangePercent = meta.regularMarketChangePercent || ((oneDayChange / previousClose) * 100)
    ...
    console.log(`Price data for ${symbol}: $${currentPrice.toFixed(2)} (1D: ${oneDayChange >= 0 ? '+' : ''}${oneDayChange.toFixed(2)})`)
