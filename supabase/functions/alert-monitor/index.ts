...
async function getStockData(symbol: string) {
  try {
    ...
    const stockDataResponse = await fetch(
      `${supabaseUrl}/functions/v1/yahoo-stock-data`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${supabaseServiceKey}`,
    ...
    if (stockDataResponse.ok) {
      ...
      if (closes.length >= 50) {
        const ma50 =
          closes.slice(0, 50).reduce((sum, price) => sum + price, 0) / 50;
        movingAverages.ma_50 = ma50;
      }

      if (closes.length >= 200) {
        const ma200 =
          closes.slice(0, 200).reduce((sum, price) => sum + price, 0) / 200;
...
async function evaluateAlertCondition(
  alert: Alert,
  stockData: StockData,
): Promise<boolean> {
...
async function sendTelegramMessage(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to send Telegram message: ${JSON.stringify(errorData)}`,
    );
  }
}
