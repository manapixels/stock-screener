...
serve(async (req) => {
  ...
  try {
    ...
    if (chartData.chart.error || !chartData.chart.result?.[0]) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to fetch stock data from Yahoo Finance' }),
        { 
          status: 404, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    const quoteSummaryData: YahooQuoteSummaryResponse = await quoteSummaryResponse.json()
    ...
    const extractValue = (obj: Record<string, YahooFinanceValue | undefined>, key: string): number | undefined => {
      const value = obj[key]
    ...
    const stockData = {
      ...
      // Market data
      marketCap: extractValue(summaryDetail, 'marketCap') || extractValue(keyStats, 'marketCap') || meta.marketCap,
      volume: quotes.volume[latestIndex],
      
      // Valuation metrics
      trailingPE: extractValue(summaryDetail, 'trailingPE') || extractValue(keyStats, 'trailingPE') || meta.trailingPE,
      forwardPE: extractValue(summaryDetail, 'forwardPE') || extractValue(keyStats, 'forwardPE'),
      ...
      // Market metrics
      beta: extractValue(summaryDetail, 'beta') || extractValue(keyStats, 'beta'),
      ...
      priceHistory: timestamps.slice(-5).map((timestamp, index) => ({
