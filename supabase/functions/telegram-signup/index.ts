...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`📝 Telegram signup request for user ${signupData.telegram_user_id}`)

    // Validate required fields
    if (!signupData.telegram_user_id || !signupData.telegram_chat_id || !signupData.first_name) {
      return new Response(JSON.stringify({
        success: false,
        message: 'Missing required fields: telegram_user_id, telegram_chat_id, first_name',
    ...
    const email = signupData.email || `telegram_${signupData.telegram_user_id}@placeholder.local`
    ...
    console.log(`✅ Successfully created Telegram account for user ${authUser.user.id}`)

    const response: SignupResponse = {
      success: true,
      user_id: authUser.user.id,
      message: 'Account created successfully! You can now use all bot features.'
    }
