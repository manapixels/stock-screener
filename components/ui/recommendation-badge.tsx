...
export function RecommendationBadge({
  ...
  return (
    <div
      className={`rounded-lg border-2 ${config.borderColor} ${config.bgLight} p-4 ${className}`}
    >
      <div className="flex items-center justify-between mb-3">
        <div
          className={`flex items-center gap-2 px-3 py-1 rounded-full ${config.color} ${config.textColor}`}
        >
          {config.icon}
      ...
      {reason && <p className={`text-sm ${config.textLight} mb-2`}>{reason}</p>}

      {upside && (
        <div className={`text-sm ${config.textLight}`}>
          <span className="font-medium">
            {upside.startsWith("-") ? "Downside" : "Upside"}:{" "}
