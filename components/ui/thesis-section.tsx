...
export function ThesisSection({ bullCase, bearCase, className = "" }: ThesisSectionProps) {
  const getStrengthIcon = (strength?: string, type: 'bull' | 'bear' = 'bull') => {
  ...
  const getStrengthColor = (strength?: string, type: 'bull' | 'bear' = 'bull') => {
    if (type === 'bull') {
      switch (strength) {
        case 'strong':
  ...
  return (
    <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 ${className}`}>
      ...
      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
        ...
        <div className="space-y-3">
          {bullCase.map((point, index) => (
            <div key={index} className="flex items-start gap-3">
              ...
              <p className={`text-sm ${getStrengthColor(point.strength, 'bull')}`}>
                {point.text}
              </p>
            </div>
          ))}
        ...
        {bullCase.length === 0 && (
          <p className="text-green-600 text-sm italic">No bullish factors identified</p>
        )}
      </div>
      ...
      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
        ...
        <div className="space-y-3">
          {bearCase.map((point, index) => (
            <div key={index} className="flex items-start gap-3">
              ...
              <p className={`text-sm ${getStrengthColor(point.strength, 'bear')}`}>
                {point.text}
              </p>
            </div>
          ))}
        ...
        {bearCase.length === 0 && (
          <p className="text-red-600 text-sm italic">No bearish factors identified</p>
        )}
      </div>
    </div>
  )
